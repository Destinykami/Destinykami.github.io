...
```
这里一开始遇到个问题，在枚举类型中,枚举是从EVENT_NULL=0开始的，而mcause对应的EVENT_YIELD也应是0，会一直把异常识别成EVENT_NULL，解决方法是简单粗暴的把EVENT_NULL给删了，这样就从EVENT_YIELD开始枚举， 不知道后续会不会有什么影响。
```c
// An event of type @event, caused by @cause of pointer @ref
typedef struct {
  enum {
    //EVENT_NULL = 0, //删除
    EVENT_YIELD=0, //从YIELD=0开始枚举
